...
export const usePermissionStore = defineStore("permission", () => {
  ...
  function generateRoutes() {
    return new Promise<RouteRecordRaw[]>((resolve) => {
      console.log(router.options, "1231242421412");

      const dynamicRoutes = transformRoutes(router.options.routes);
  ...
  const setMixLeftMenus = (topMenuPath: string) => {
    const matchedItem = router.options.routes.find((item) => item.path === topMenuPath);
